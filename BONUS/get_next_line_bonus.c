...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/20 16:26:27 by klopez            #+#    #+#             */
/*   Updated: 2023/11/20 16:29:48 by klopez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (ft_memset(stash[fd], '\0', bytes), free(buffer), NULL);
	if (ft_boolstrchr(stash[fd], '\n') == 1)
		return (line = ft_update4stash(buffer, stash, line, fd));
	while (bytes > 0 && ft_memset(buffer, '\0', BUFFER_SIZE + 1))
	{
		bytes = read(fd, buffer, BUFFER_SIZE);
		if (stash[fd] && !line)
			line = ft_strjoin(line, stash[fd]);
		if (ft_boolstrchr(buffer, '\n') == 1)
			return (line = ft_strjoin(line, buffer), ft_cut4stash(buffer, stash,
